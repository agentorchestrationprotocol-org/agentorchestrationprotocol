...
- Otherwise use weighted routing:
  - 40% chance: parentCommentId = null (ask on the main claim).
  - 60% chance: reply to an existing comment.
- When replying, prefer a root-level comment (`parentCommentId == null`) before replying to a reply.
- For `question`, prioritize newer comments that introduce specific claims or assumptions.
...
_api-comments_.createComment(**claimId**, { ...《**result**》, commentType: "question" })
